...
// Лёгкая маска телефона (добавлено)
...
form?.addEventListener('submit', (e) => { 
    ...
    if (!form.checkValidity()) { 
        ...
        // Пример: таргетированное сообщение для email (можно добавить для других полей) 
        const email = form.elements.email; 
        if (email?.validity.typeMismatch) { 
            email.setCustomValidity('Введите корректный e-mail, например name@example.com'); 
        } 
        // Добавьте аналогичные проверки для других полей, если нужно (например, для телефона или имени)
        const phone = form.elements.phone;
        if (phone?.validity.patternMismatch) {
            phone.setCustomValidity('Формат телефона: +7 (900) 000-00-00');
        }
